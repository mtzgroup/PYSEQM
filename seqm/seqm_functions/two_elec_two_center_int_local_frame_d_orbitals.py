...
####
        ...
        if (drho_corea.sum() > 0 or drho_coreb.sum() > 0 ):
            ...
            Sq_DSQtzx_onehalfQtxy = ev2/torch.sqrt((r0-dsb0)**2+rhoSSD)+ev2/torch.sqrt((r0+dsb0)**2+rhoSSD)-ev1/torch.sqrt((r0)**2+(dsb0)**2+rhoSDS)
            #############################
            DSQtzx_onehalfQtxy_Sq = ev2/torch.sqrt((r0-dsa0)**2+rhoSDS)+ev2/torch.sqrt((r0+dsa0)**2+rhoSDS)-ev1/torch.sqrt((r0)**2+(dsa0)**2+rhoSSD)
