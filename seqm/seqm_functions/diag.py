...
# if yes, then equal contributions are used for degenerate orbitals near fermi level
...
def sym_eig_trunc1(x,nheavyatom,nH,nocc, eig_only=False):
    ...
    if x.dim()==2:
        ...
        e0, v0 = list(zip(*list(map(
                        lambda a,b,c: sym_eigh(pack(a,b,c)),#, UPLO='U'),
                        x,nheavyatom, nH))))
        if CHECK_DEGENERACY:
            P0 = list(map(
