...
class additive_term_rho1(torch.autograd.Function):
    ...
    def forward(ctx, hsp_ev, D1):
        ...
        c = hsp < 0.0
        D1_sq = D1 * D1
        d1 = (torch.abs(hsp) / D1_sq)**(1.0/3.0)
        d1[c] *= -1.0

        d2 = d1 + 0.04
        for i in range(1,6):
            hsp1 = 0.5*d1 - 0.5/torch.sqrt(4.0*D1_sq+1.0/d1**2)
            hsp2 = 0.5*d2 - 0.5/torch.sqrt(4.0*D1_sq+1.0/d2**2)
...
#                break
    ...
    def backward(ctx, grad_output):
        ...
        dD1 = grad_output * (tmp / rho1**2 - rho1) / D1
        del tmp
        return (dhsp, dD1)
...
class additive_term_rho2(torch.autograd.Function):
    ...
    def forward(ctx, hpp_ev, D2):
        ...
        c = hpp<0.0
        D2_sq = D2 * D2
        q1 = (torch.abs(hpp)/3.0/D2_sq**2)**0.2
        q1[c] *= -1.0

        q2 = q1 + 0.04
        for i in range(1,6):
            hpp1 = 0.25*q1 - 0.5/torch.sqrt(4.0*D2_sq+1.0/q1**2) + 0.25/torch.sqrt(8.0*D2_sq+1.0/q1**2)
            hpp2 = 0.25*q2 - 0.5/torch.sqrt(4.0*D2_sq+1.0/q2**2) + 0.25/torch.sqrt(8.0*D2_sq+1.0/q2**2)
...
#                break
    ...
    def backward(ctx, grad_output):
        ...
        D2_sq, rho2_sq = D2 * D2, rho2 * rho2
        tmp1 = 1.0 / (D2_sq + rho2_sq)**1.5
        tmp2 = 1.0 / (2.0 * D2_sq + rho2_sq)**1.5
        dhppdrho2 = -0.125 / rho2_sq + rho2 * (tmp1 / 4.0 - tmp2 / 8.0)
        go_dhppdrho2 = grad_output * dhppdrho2
        dhpp_ev = go_dhppdrho2 * ev
        dD2 = -go_dhppdrho2 / (D2 / 4.0 * (tmp1 - tmp2))
