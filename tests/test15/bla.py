...
from seqm.basics import Energy, Force
import seqm
...
with torch.autograd.set_detect_anomaly(True):
    ...
    grad_bf, = torch.autograd.grad(Etot.sum(), coordinates, create_graph=True)
    Fz_bf = -grad_bf[:,0,2]
    H_bf, = torch.autograd.grad(Fz_bf.sum(), coordinates)
    Hzz_bf = H_bf[:,0,2]
    
    seqm_parameters["2nd_grad"] = True
    force = Force(seqm_parameters).to(device)
    F_in, P, Etot2, HF, Eel, Enuc, Eiso, EnucAB, e, charge, nconv = force(const, coordinates, species, mode="new")
    print("Diff in Etot = ",(Etot-Etot2).abs().sum())
    Fz_in = F_in[:,0,2]
    H_in, = torch.autograd.grad(Fz_in.sum(), coordinates)
    Hzz_in = H_in[:,0,2]
    

f=open('log.dat', 'w')
f.write("#index, dx (Angstrom), Etot (eV), Fz_bf (eV/Angstrom), Fz_in (eV/Angstrom), Hzz_bf (eV/Angstrom^2), Hzz_in (eV/Angstrom^2)\n")
for i in range(N):
    f.write("%d %12.8e %12.8e %12.8e %12.8e %12.8e %12.8e\n" % (i,dx[i],Etot[i],Fz_bf[i],Fz_in[i],Hzz_bf[i],Hzz_in[i]))
f.close()
