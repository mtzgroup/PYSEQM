            ...
            for mol in self.output['molid']:
                ...
                if e_gap.dim()==1:
                    f.write("{}\nstep: {}  T= {:12.3f}K  Ek= {:12.6f}  Ep= {:12.6f}  E_gap= {:12.6f}  Err= {:24.16f}  time_stamp= {:.4f}\n".format(
                                               torch.sum(molecule.species[mol]>0), i+1, T[mol], Ek[mol], L[mol], e_gap[mol], Err[mol], time.time()))
                else:
                    f.write("{}\nstep: {}  T= {:12.3f}K  Ek= {:12.6f}  Ep= {:12.6f}  E_gap= {:12.6f}/{:12.6f}  Err= {:24.16f}  time_stamp= {:.4f}\n".format(
                                        torch.sum(molecule.species[mol]>0), i+1, T[mol], Ek[mol], L[mol], e_gap[mol,0], e_gap[mol,1], Err[mol], time.time()))
